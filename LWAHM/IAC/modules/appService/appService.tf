...
resource "azurerm_linux_web_app_slot" "app_service_staging_slot" {
  name                            = "staging-slot"
  ...
  site_config {
    ...
    application_stack {
      docker_registry_url = "https://acr${var.platform}${var.environment}.azurecr.io"
      docker_image_name   = "${var.image_name}:${var.staging_image_tag}"
    }
  }
}

resource "azurerm_app_service_slot_virtual_network_swift_connection" "vnet_integration_staging_slot" {
  slot_name      = azurerm_linux_web_app_slot.app_service_staging_slot.name
  app_service_id = azurerm_linux_web_app.app_service.id
